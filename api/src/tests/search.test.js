...
describe('API Endpoints', () => {
    ...
    describe('GET /api/search', () => {
        it('should return a 400 if query (q) is missing', async () => {
            const response = await (0, supertest_1.default)(app_1.default).get('/api/search?lat=6.13&lng=1.22');
            expect(response.statusCode).toBe(400);
            expect(response.body.error).toBe('Search query is required');
        });
        it('should return a 400 if lat or lng are missing', async () => {
            const response = await (0, supertest_1.default)(app_1.default).get('/api/search?q=paracetamol');
            expect(response.statusCode).toBe(400);
        ...
        it('should return an empty array for a valid query (if db is empty or not connected)', async () => {
            const response = await (0, supertest_1.default)(app_1.default).get('/api/search?q=unlikelymed&lat=6.13&lng=1.22');
