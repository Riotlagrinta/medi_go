...
function PaiementContent() {
      ...
      const fetchReservationDetails = async (id: number) => {
        try {
          ...
          if (res.ok) {
            ...
            // Supposons que l'API retourne un tableau d'items et un total
            setOrderSummary(data.items || []);
            setTotal(data.total_amount || 0);
          }
        } catch (e) {
          setError("Impossible de charger les détails de la commande.");
          // Fallback avec les données de l'URL
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      if (method === 'card') {
        // Alerte temporaire car la fonctionnalité n'est pas implémentée
        alert("Le paiement par carte bancaire n'est pas encore fonctionnel. Cette fonctionnalité est en cours de développement.");
        setLoading(false);
        return;
      }
